...
export default function ReportScreen({navigation}) {
    ...
    try {
      ...
      const items = querySnapshot.docs.map(doc => {
        ...
        return {
          ...
          add_minus: parseFloat(data.add_minus),
        };
      });
  ...
  const handleSortTipe = () => {
    const direction =
      sortConfig.column === 'Tipe' && sortConfig.direction === 'asc'
        ? 'desc'
        : 'asc';
    setSortConfig({column: 'Tipe', direction});
  };

  const handleSortTotalItem = () => {
    const direction =
      sortConfig.column === 'Banyak Barang' && sortConfig.direction === 'asc'
        ? 'desc'
        : 'asc';
    setSortConfig({column: 'Banyak Barang', direction});
  };
  const handleSortAddMinusItem = () => {
    const direction =
      sortConfig.column === 'Masuk/Keluar' && sortConfig.direction === 'asc'
        ? 'desc'
        : 'asc';
    setSortConfig({column: 'Masuk/Keluar', direction});
  };

  const handleSortDate = () => {
    const direction =
      sortConfig.column === 'Tanggal' && sortConfig.direction === 'asc'
        ? 'desc'
        : 'asc';
    setSortConfig({column: 'Tanggal', direction});
  };
  ...
  const exportToExcel = async () => {
    try {
      ...
      if (sortedAndFilteredData.length === 0) {
        Alert.alert(
          'Tidak Ada Data',
          `Tidak ada data untuk tanggal ${dataDownload.from} hingga ${dataDownload.to}`,
        );
        return;
      }
  ...
  const exportAllToExcel = async () => {
    try {
      ...
      const sortedAndFilteredData = sortedData.filter(item => {
        if (dataDownload.type === 'in') {
          return item.Tipe === 'in';
        } else if (dataDownload.type === 'out') {
          return item.Tipe === 'out';
      ...
      const xlsxname = `Semua Laporan.xlsx`;
  ...
  useEffect(() => {
    if (sortedData) {
      const sorted = [...sortedData].sort((a, b) => {
        if (sortConfig.column === 'Tipe') {
          const tipeA = a.Tipe || '';
          const tipeB = b.Tipe || '';
          return sortConfig.direction === 'asc'
            ? tipeA.localeCompare(tipeB)
            : tipeB.localeCompare(tipeA);
        } else if (sortConfig.column === 'Banyak Barang') {
          return sortConfig.direction === 'asc' ? a.qty - b.qty : b.qty - a.qty;
        } else if (sortConfig.column === 'Tanggal') {
          return sortConfig.direction === 'asc'
            ? moment(a.date, 'DD-MM-YYYY').diff(moment(b.date, 'DD-MM-YYYY'))
            : moment(b.date, 'DD-MM-YYYY').diff(moment(a.date, 'DD-MM-YYYY'));
        } else if (sortConfig.column === 'Masuk/Keluar') {
          return sortConfig.direction === 'asc'
            ? a.add_minus - b.add_minus
            : b.add_minus - a.add_minus;
        } else {
          return moment(a.date, 'DD-MM-YYYY').isAfter(
        ...
        <>
          <View style={styles.cardView}>
            ...
            <Button
              ...
              disabled={dataDownload.type === '' ? true : false}
              onPress={async () => {
                exportAllToExcel();
              }}>
              Unduh Semua Data Laporan
            </Button>
          </View>
        </>
      ) : (
              ...
              <DataTable>
                ...
                {sortedData?.slice(from, to).map((rowData, rowIndex) => (
                  <DataTable.Row key={rowData.id}>
                    <DataTable.Cell
                      style={{width: cellWidths[0], justifyContent: 'center'}}>
                      {from + rowIndex + 1} {/* Calculate the actual index */}
                    </DataTable.Cell>
                    <DataTable.Cell
                      style={{width: cellWidths[1], justifyContent: 'center'}}>
                      {rowData.name}
                    </DataTable.Cell>
                    <DataTable.Cell
                      style={{width: cellWidths[2], paddingLeft: 5}}>
                      {rowData.Tipe}
                    </DataTable.Cell>
                    <DataTable.Cell
                      style={{
                    ...
                    <DataTable.Cell
                      style={{width: cellWidths[5], justifyContent: 'center'}}>
                      {rowData.person}
                    </DataTable.Cell>
                    <DataTable.Cell
                      style={{
                        width: cellWidths[6],
                        paddingLeft: screenWidth * 0.22,
                      }}>
                      {rowData.nota}
                    </DataTable.Cell>
                  </DataTable.Row>
