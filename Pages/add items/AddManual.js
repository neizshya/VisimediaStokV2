...
export default function AddManual({navigation}) {
  ...
  const handleOnPress = async () => {
    ...
    try {
      ...
      const itemIdQuery = await getDocs(
        query(itemsRef, where('id', '==', dataItem.itemId)),
      );
      const itemNameQuery = await getDocs(
        query(itemsRef, where('item_name', '==', dataItem.itemName)),
      );

      if (!itemIdQuery.empty) {
        alert('Barang dengan ID tersebut sudah ada dalam database');
        setIsLoading(false);
        return;
      }

      if (!itemNameQuery.empty) {
        alert('Barang dengan nama tersebut sudah ada dalam database');
        setIsLoading(false);
        return;
      }
      ...
      <View style={styles.cardView}>
        <View style={{width: screenWidth * 0.8}}>
          <View style={{marginVertical: 5}}>
            <Text style={{color: 'black'}}>Id Barang:</Text>
            <TextInput
              mode="outlined"
              style={{height: 30}}
              value={dataItem.itemId}
              onChangeText={text => setDataItem({...dataItem, itemId: text})}
            />
          </View>
          <View style={{marginVertical: 5}}>
            ...
            <TextInput
              ...
              onChangeText={text => setDataItem({...dataItem, itemName: text})}
            />
          </View>
          {/* qty */}
          <View style={{marginBottom: 10}}>
            <Text style={{marginBottom: 10, color: 'black'}}>Qty:</Text>
            <NumericInput
              rounded
              totalWidth={100}
              totalHeight={40}
          ...
          <View style={{marginTop: 10}}>
            <Text style={{marginBottom: 10, color: 'black'}}>Foto Barang</Text>
            <Button
              buttonColor="#5689c0"
              style={{
                width: 150,
          ...
          <View style={{marginVertical: 10}}>
            <Text style={{color: 'black'}}>Pengirim:</Text>
            <TextInput
              mode="outlined"
              style={{height: 30}}
              value={dataItem.person}
              onChangeText={text => setDataItem({...dataItem, person: text})}
            />
          </View>
          <View style={{marginVertical: 10}}>
